...
const mealSections = [
  { id: 'breakfast', label: 'Breakfast', emoji: '🌅', goal: 500 },
  { id: 'lunch', label: 'Lunch', emoji: '☀️', goal: 650 },
  { id: 'dinner', label: 'Dinner', emoji: '🌙', goal: 600 },
  { id: 'snack', label: 'Snacks', emoji: '🍿', goal: 250 },
];
...
export default function Diary({ currentPage, onNavigate }: DiaryProps) {
  const logged: Record<string, { food: typeof sampleFoods[0]; quantity: number; time: string }[]> = {
    breakfast: [{ food: sampleFoods[7], quantity: 1, time: '8:30 AM' }, { food: sampleFoods[5], quantity: 1, time: '8:35 AM' }],
    lunch: [{ food: sampleFoods[3], quantity: 1, time: '12:45 PM' }],
    dinner: [],
    snack: [{ food: sampleFoods[4], quantity: 1, time: '3:30 PM' }],
  };
  ...
  return (
    <AppShell currentPage={currentPage} onNavigate={onNavigate} title="Food Diary" subtitle="Today, July 26" showSearch>
      {/* Summary card */}
      <div className="card mb-4 sm:mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-text-primary">Today's summary</h3>
          <Flame size={18} className="text-accent" />
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <div className="text-center">
            <p className="text-2xl font-bold text-primary">{totalEaten}</p>
            <p className="text-xs text-text-secondary">Calories eaten</p>
          </div>
          <div className="text-center">
          ...
          <div className="text-center">
            <p className="text-2xl font-bold text-primary">{Math.round(protein)}g</p>
            <p className="text-xs text-text-secondary">Protein</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-info">{Math.round(carbs)}g</p>
          ...
          return (
            <div key={section.id} className="card">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-sm text-text-secondary">{sectionCals} / {section.goal} kcal</span>
              </div>
              ...
              {entries.length === 0 ? (
                ...
                >
                  <Plus size={16} /> Add food to {section.label.toLowerCase()}
                </button>
              ) : (
      ...
      <div className="card mt-3 sm:mt-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-sm text-text-secondary">6 / 8 glasses</span>
        </div>
      ...
      <div className="flex items-start gap-2.5 sm:gap-3 bg-gradient-to-r from-primary-50 to-accent-50 rounded-2xl p-3 sm:p-4 mt-3 sm:mt-4">
        ...
        <div className="flex-1 pt-1">
          <p className="text-sm text-text-primary leading-relaxed">
            Nice balance today! Your protein intake is looking great. For dinner, a piece of grilled salmon would round out your omega-3s nicely. 🐟
          </p>
        </div>
      </div>
    </AppShell>
